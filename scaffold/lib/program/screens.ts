...
export interface TemplateHead { id: string; code: string; name: string; template_kind: string; kind_label: string; asset_class: string | null; current_version_id: string | null; is_active: boolean; hide_record_from_subject: boolean }
...
export async function loadProgramScreen(id: string, versionParam: string | undefined): Promise<{ template: TemplateHead; program: LoadedProgram | null; versionQuery: string; wantedVersion: string | null }> {
  ...
  const template = (await query<TemplateHead>(`
    SELECT t.id, t.code, t.name, t.template_kind, COALESCE(k.label, t.template_kind) AS kind_label, ac.code AS asset_class, t.current_version_id, t.is_active,
           COALESCE((SELECT v.hide_record_from_subject FROM session_template_versions v WHERE v.id = COALESCE($2::uuid, t.current_version_id)), false) AS hide_record_from_subject
     ...
     WHERE t.id = $1::uuid AND t.deleted_at IS NULL`, [id, wantedVersion]))[0] ?? null;
