...
const courses = [];
for (const course of R.initial_training?.courses ?? []) {
  for (let i = 0; i < course.size; i += 1) {
    seniority += 1;
    const sex = rng.chance(R.female_share) ? 'F' : 'M';
    ...
    courses.push({
      ...
      licence_number: `${R.licence.FO}${String(100000 + seniority * 37 + rng.int(0, 36)).slice(-6)}`,
      instructor_roles: [], roster_status: 'active', training_course: course.code,
    });
  }
}

const out = { note: 'Pilot roster generated by scripts/gen-roster.mjs from policy.yaml `roster`. Every person is invented. No training data.', seed: R.seed, as_of: R.as_of, pilots: [...pilots, ...courses, ...candidates] };
...
console.log(`${pilots.length} pilots (${R.ranks.map((r) => `${r.count} ${r.fleet} ${r.position}`).join(', ')}), ${candidates.length} candidates, ${courses.length} in initial training (${(R.initial_training?.courses ?? []).map((c) => c.code).join(', ')}); bench ${Object.entries(bench).map(([k, v]) => `${k} ${v}`).join(', ')}; ${pilots.filter((p) => p.sex === 'F').length} women`);
