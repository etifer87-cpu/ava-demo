...
function lfus() {
  // One record per sector. The nine phases of a line sector, each graded as a task 1-5; then the
  // overall grading on every competency with observable behaviours. Take-off, approach and
  // landing carry the PF/PM choice, and take-offs and landings as PF count on the LFUS status.
  const phase = (key, title, time, text, criteria, pfPm = null) => section(key, title, null, time, {}, [
    task(`${key}.task`, title, { time, ...(pfPm ? { pf_pm: pfPm } : {}), aims: { aims: text, visibility: 'also_on_report', grading_criteria: criteria }, grading: grading('scale_1_5') }),
  ]);
  return {
    note: 'Line flying under supervision, every fleet, one record per sector. Sector number, departure, arrival, block and night time, purpose and the other pilot are entered by the instructor when the session is created; the template carries the phases, the PF/PM flags and the overall grading. Generated by scripts/gen-programs.mjs.',
    template: {
      code: 'lfus.sector', name: 'LFUS - line flying under supervision', kind: 'line_supervised', fleet: null,
      setup: { program: { code: 'LFUS', module: 'Line flying under supervision', phase: null, day: null, cycle_months: null, year: null }, period: null, aims: aims('Consolidate the type rating or the upgrade on the line under a line training captain; one record per sector, competencies observed across the sector.') },
      allowed_assessor_roles: ROLES.ebt,
      notes: 'Sector details are session fields. Take-offs and landings as PF are counted from the PF/PM choice on those phases.',
    },
    elements: [
      phase('ground', 'Aircraft ground and pre-flight operations', '0:45', 'Flight preparation, documents, walk-around, cockpit preparation and the departure briefing.', 'Preparation complete and on time; briefing covers threats, plan and monitoring.'),
      phase('takeoff', 'Take-off', '0:10', 'Line-up, take-off roll, rotation and initial climb.', 'Centreline kept; rotation and pitch as per SOP; callouts complete.', 'take_off'),
      phase('climb', 'Climb', '0:20', 'Climb management, ATC compliance, altitude and speed constraints.', 'Constraints met; automation modes understood and announced.'),
      phase('cruise', 'Cruise', '0:30', 'Fuel and systems monitoring, weather, cabin coordination, arrival preparation and briefing.', 'Monitoring maintained; arrival briefed in time; threats identified.'),
      phase('descent', 'Descent', '0:20', 'Descent planning and energy management to the approach.', 'Descent profile managed; constraints met; configuration on schedule.'),
      phase('approach', 'Approach', '0:15', 'Approach as briefed; stabilised approach criteria.', 'Stabilised by 1 000 ft; deviations called and corrected; go-around when required.', 'other'),
      phase('landing', 'Landing', '0:05', 'Flare, touchdown and rollout.', 'Touchdown in the zone on the centreline; rollout and deceleration controlled.', 'landing'),
      phase('postflight', 'After-landing and post-flight operations', '0:15', 'Taxi in, shutdown, documents and reports.', 'Aircraft secured and handed over; reports filed.'),
      phase('misc', 'Miscellaneous', '0:10', 'Anything observed outside the phases: SOP knowledge, commercial awareness, crew coordination.', 'As observed.'),
      section('overall', 'Overall grading', null, null, {}, [
        task('overall.task', 'Overall grading - competencies', { aims: { aims: 'The sector as a whole, on every competency, with the observable behaviours that drove the grade.', visibility: 'also_on_report', grading_criteria: 'Grade 3 or above on every competency is the expected line standard.' }, grading: grading('none', 'scale_1_5', ALL_COMPS) }),
      ]),
    ],
    competencies_default: ALL_COMPS,
  };
...
function screening() {
  // Recruitment: an internal record the candidate never sees. Two scenarios to choose from
  // (SKBO, SKRG), a small malfunction grid, one assessed exercise with the assessor's comments,
  // then the overall grading on every competency with observable behaviours.
  const f = FLEET.A320;
  return {
    note: 'Pilot screening, A320 FFS. Internal record (hidden from the candidate): assessor signature only. Two set-ups to choose from, a short malfunction grid, one assessed exercise, overall competencies with observable behaviours. Generated by scripts/gen-programs.mjs; every value is illustrative.',
    template: {
      code: 'screen.a320', name: 'Pilot screening - A320', kind: 'assessment', fleet: 'A320',
      setup: { program: { code: 'SCREEN', module: 'Screening', phase: null, day: null, cycle_months: null, year: null }, period: '1:30', aims: aims('Assess a candidate\'s handling, procedures and behaviours in a short line-like scenario; an internal assessment, not training.') },
      allowed_assessor_roles: ROLES.ebt,
      hide_record_from_subject: true,
      notes: 'Internal assessment: the candidate does not see the record.',
    },
    elements: [
      section('brief', 'Briefing', 'brief', '0:30', {}, [
      ...
      section('scenario', 'Scenario', null, '1:30', { aims: { aims: 'Choose ONE scenario. Both are a short domestic sector with a manual departure, a system failure of the assessor\'s choice and an approach to a landing.', visibility: 'instructor_only' } }, [
        ...
        setup('scenario.skrg', 'Scenario B - SKRG to SKBO', { airport: ['SKRG', 'SKBO'], weather: [wx('SKRG', 0), wx('SKBO', 0)], position: [`SKRG RWY 01, ${f.position.lineup}`], comms: [COMMS.SKRG[1], COMMS.SKBO[2]], reset: ['Total reset'], atc: ['Standard departure; RNP approach at SKBO'], performance: [f.perf.toga] }, f.mass(f.fuel.mt), 'take', 'Terrain-rich departure, manual flight to FL200, then automation as briefed.'),
        malfGrid('scenario.malf', 'System failure', 'A320', [{ ref: 'malf.a320.29.eng-pump-lo-press', option: 'Green', trigger: 'After gear retraction' }, { ref: 'malf.a320.24.gen-fault-excitation', option: 'Gen1', trigger: 'In the climb' }, { ref: 'malf.a320.21.pack-regul-fault', option: '1', trigger: 'Passing FL150' }, { ref: 'malf.a320.70.flame-out-no-damage', option: '2', trigger: 'At V1' }]),
        events('scenario.events', 'Events', [EVENTS.reclear, EVENTS.crosswind]),
        task('scenario.flight', 'Assessed flight', { time: '1:30', pf: 'CM1', automation: { ap: 'crew_discretion', athr: 'crew_discretion', fd: 'required_on' }, aims: aims('Fly the sector as briefed: manual departure, manage the failure with the assessor as PM, approach and landing.', 'Basic handling and scan; procedure discipline as briefed; communication and workload under a failure; decision making on the approach.', 'Safe handling throughout; the failure managed with the assessor\'s support; a stabilised approach or a go-around decided in time.', 'instructor_only'), conduct: { instructor_notes: 'The assessor acts as PM and ATC. Insert the failure once the candidate is settled after departure. Comment freely: the comments are what the selection board reads.', injects: [] }, grading: grading('scale_1_5') }),
      ]),
      section('overall', 'Overall assessment', null, null, {}, [
        task('overall.task', 'Overall assessment - competencies', { aims: { aims: 'The session as a whole on every competency, with the observable behaviours seen; the assessor\'s comments complete the picture for the selection board.', visibility: 'instructor_only', grading_criteria: 'Grade 3 or above on every competency is the expected entry standard.' }, grading: grading('none', 'scale_1_5', ALL_COMPS) }),
      ]),
    ],
    competencies_default: ALL_COMPS,
  };
...
function typeRatingGround(fleet, year) {
  const subjects = [
    ['intro', 'Aircraft general and flight deck familiarisation', '4:00', 'Dimensions, flight deck layout, displays, the philosophy of the type.'],
    ['electrical', 'Electrical and pneumatic systems', '4:00', 'Generation, distribution, bleed and pressurisation; normal and abnormal operation.'],
    ['hydraulic', 'Hydraulics, landing gear and brakes', '3:00', 'Systems, indications, degraded configurations and their landing consequences.'],
    ['controls', 'Flight controls and automation', '5:00', 'Control laws or modes, protections, autoflight, the flight management system.'],
    ['engines', 'Powerplant, fuel and APU', '4:00', 'Engine indications, fuel system and management, APU operation.'],
    ['performance', 'Performance and mass and balance', '4:00', 'Take-off and landing performance, contaminated runways, mass and balance.'],
    ['procedures', 'Normal and abnormal procedures', '5:00', 'SOPs, flows and callouts; QRH and ECAM or checklist philosophy; memory items.'],
    ['exam', 'Type rating knowledge examination', '2:00', 'Written examination across all subjects; 75 % is the pass mark.'],
  ];
  const total = subjects.reduce((m, x) => m + Number(x[2].split(':')[0]) * 60 + Number(x[2].split(':')[1]), 0);
  return {
    note: `Type rating ground school, ${fleet}, ${year}: one record at the end of the ground phase, each subject graded as a task 1-5 and on knowledge. Generated by scripts/gen-programs.mjs; every value is illustrative.`,
    template: {
      code: `tr.${fleet.toLowerCase()}.${year}.ground`, name: `Type rating ground school - ${fleet}`, kind: 'ground_school', fleet,
      setup: { program: { code: `TR-${fleet}`, module: 'Type rating', phase: 'Ground', day: null, cycle_months: null, year }, period: `${Math.floor(total / 60)}:${String(total % 60).padStart(2, '0')}`, aims: aims('The systems and procedures knowledge the simulator phase builds on.') },
      allowed_assessor_roles: ROLES.ground,
      notes: 'Initial training. Precedes the FFS phase.',
    },
    elements: subjects.map(([key, title, time, text], i) => section(key, title, null, time, {}, [
      task(`${key}.subject`, title, { time, aims: { aims: text, visibility: 'also_on_report', grading_criteria: i === subjects.length - 1 ? '75 % or above.' : 'Answers the instructor\'s questions correctly; completes the exercises.' }, grading: grading('scale_1_5', 'competent_not_competent', ['KNO']) }),
    ])),
    competencies_default: ALL_COMPS,
  };
}

function typeRatingFfs(fleet, year, n, total) {
  const f = FLEET[fleet];
  // A progression: handling and normal procedures first, then the abnormal syllabus, then LOFT-style sessions before the skill test.
  const plan = [
    ['Normal procedures and handling', ['rawData', 'gaAll', 'crosswind']],
    ['Handling, automation and navigation', ['nonPrecision', 'lvo', 'rawData']],
    ['Engine failures', ['engV1', 'oeiGa', 'rto']],
    ['Engine failures and OEI landing', ['engV1', 'oeiLdg', 'oeiGa']],
    ['Systems abnormals', ['emergencyDescent', 'stall', 'gaAll']],
    ['Systems abnormals and upset', ['upset', 'windshear', 'egpws']],
    ['Line-oriented session', ['tcasRa', 'nonPrecision', 'crosswind']],
    ['Skill test preparation', ['engV1', 'oeiGa', 'lvo']],
  ][n - 1] ?? ['Session', ['rawData', 'gaAll', 'crosswind']];
  const [title, ids] = plan;
  const ex = ids.flatMap((id, i) => manoeuvre(fleet, id, `s.ex${i + 1}`, i === 0 ? '1:20' : i === 1 ? '1:20' : '1:00', i % 2 === 0 ? 'CM1' : 'CM2', n + i));
  return {
    note: `Type rating FFS session ${n} of ${total}, ${fleet}, ${year}: ${title.toLowerCase()}. Each exercise graded as a task 1-5. Generated by scripts/gen-programs.mjs; every value is illustrative.`,
    template: {
      code: `tr.${fleet.toLowerCase()}.${year}.ffs${n}`, name: `Type rating FFS ${n} - ${fleet}`, kind: 'type_rating', fleet,
      setup: { program: { code: `TR-${fleet}`, module: 'Type rating', phase: 'FFS', day: n, cycle_months: null, year }, period: '4:00', aims: aims(`FFS session ${n} of ${total}: ${title.toLowerCase()}.`) },
      allowed_assessor_roles: ROLES.ebt,
      notes: `Initial training, simulator phase, session ${n} of ${total}.`,
    },
    elements: [
      section('brief', 'Briefing', 'brief', '1:00', {}, [
...
defs.push(lfus());
defs.push(screening());
defs.push(typeRatingGround('A320', 2026));
for (let n = 1; n <= 8; n += 1) defs.push(typeRatingFfs('A320', 2026, n, 8));
