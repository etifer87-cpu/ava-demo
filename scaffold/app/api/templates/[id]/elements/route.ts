...
import { addBlank, addFromLibrary, addSection, moveElement, removeElement, renameElement, updateSection, WriteRefused } from '@/lib/program/write';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const back = (flash: Parameters<typeof flashCookie>[0], sel: string | null) => {
  ...
  try {
    ...
    return back({ kind: 'bad', message: err instanceof Error ? err.message : 'The change was not made.' }, key || null);
  }
}
