...
| `/templates/[id]` | `app/(training)/templates/[id]/page.tsx` | The builder: library rail, program canvas with `?sel=` selection, inspector (structure forms), content problems, findings | `training.templates.view` | built | `template-builder` |
...
| `/api/templates/[id]/elements` | `app/api/templates/[id]/elements/route.ts` | POST structural changes to a draft: add_section, add_blank, add_library, set_section, move, rename, remove; one transaction and one audit row each | `training.templates.configure` | built | - |
