...
| `/sessions` | `app/(training)/sessions/page.tsx` | Every session in reach: status tabs with counts, filters by kind/fleet, search over program, pilot and instructor | `training.sessions.view` (fleet-bound) | built | `session-list` |
| `/sessions/mine` | `app/(training)/sessions/mine/page.tsx` | The instructor's own queue grouped to grade / planned / awaiting a signature / finalised | `training.sessions.grade` | built | `my-session-list` |
| `/sessions/[id]` | `app/(training)/sessions/[id]/page.tsx` | One session: the frozen identity, the pilot and seat, signature state, and the record once finalised | `training.sessions.view` | built | `session-detail` |
| `/sessions/new` | `app/(training)/sessions/new/page.tsx` | Open a session: published programs the caller's qualifications and fleet allow, date, device or route, which check, pilot and seat | `training.sessions.create` | built | `session-new` |
| `/api/sessions` | `app/api/sessions/route.ts` | POST create one session; every eligibility rule re-checked server-side, one `session.create` audit row | `training.sessions.create` | built | - |
...
| `/subjects/status` | `app/(training)/subjects/status/page.tsx` | Training status: line pilots' validity per item (policy `training_status.items`); initial-training board (`?group=initial`, `?view=board|table`), cards placed by milestone from lib/training/initial.ts | `training.records.view` | built | `subject-status` |
| `/instructors` | `app/(training)/instructors/page.tsx` | The bench: instructors with 12-month activity and the adjusted leniency delta from `av_assessor_adjusted`; the holder is excluded by the resolver | `training.analytics.assessor.view` | built | `assessor-analytics` |
| `/instructors/analysis` | `app/(training)/instructors/analysis/page.tsx` | Bench-wide standardisation: ASI histogram (chart 32), adjusted-delta scatter with the review threshold (33), per-competency bias grid labelled RAW (34), and the instructors outside the band | `training.analytics.assessor.view` | built | `instructor-analysis` |
| `/instructors/[id]` | `app/(training)/instructors/[id]/page.tsx` | One instructor's grading profile: adjusted delta, standardisation index and terms, competency residuals (raw), grade distribution, habits, OB habits, monthly residual, records signed | `training.analytics.assessor.view` (row-checked; never the holder) | built | `instructor-profile` |
| `/instructors/[id]/analysis` | `app/(training)/instructors/[id]/analysis/page.tsx` | One instructor: adjusted leniency with its interval against the bench distribution (chart 35), check versus training on residuals with a Welch interval, the index breakdown (39), and the alert queue (41) | `training.analytics.assessor.view` (row-checked; never the holder) | built | `instructor-analysis-detail` |
| `/api/instructors/[id]/alerts` | `app/api/instructors/[id]/alerts/route.ts` | POST one alert decision: confirmed / dismissed / reopened, reviewer note required by the route and by a CHECK, one audit row | `training.analytics.assessor.view` on the instructor **and** `training.records.amend` | built | - |
| `/api/records/[id]` | `app/api/records/[id]/route.ts` | GET one record in the pop-up's shape, for the initial-training board's sector click; 404 for anything out of reach | `training.records.view` + `people.view` on the subject | built | - |
| `/templates` | `app/(training)/templates/page.tsx` | Programs: every template with kind, fleet, current version, status and what it holds; Create new program | `training.templates.view` | built | `template-list` |
| `/templates/archive` | `app/(training)/templates/archive/page.tsx` | Archived programs (is_active = false) with Restore and the same re-authenticated Delete | `training.templates.view` | built | `template-archive` |
| `/templates/new` | `app/(training)/templates/new/page.tsx` | Create a program: identity that freezes at creation plus the version-level set-up; creates version 1 as a draft | `training.templates.configure` | built | `template-new` |
| `/templates/[id]` | `app/(training)/templates/[id]/page.tsx` | The builder: palette, drag-and-drop canvas with inline rename (client), inspector, content problems, findings | `training.templates.view` | built | `template-builder` |
| `/templates/[id]/instructor` | `app/(training)/templates/[id]/instructor/page.tsx` | The program as the instructor sees it: rail of sections and steps, one step at a time (`?at=`), grade controls inert; the instructor projection with `runtime: null` | `training.templates.view` | built | `instructor-view` |
| `/templates/[id]/review` | `app/(training)/templates/[id]/review/page.tsx` | The record as it will be signed: subject projection, outcome, both signature statements from policy.yaml, the objection dialog. The PDF is this page | `training.templates.view` | built | `review-view` |
| `/api/templates` | `app/api/templates/route.ts` | POST create (template + version 1 draft); archive / unarchive (ids); delete (ids + reason + the caller's password, re-verified by Postgres; refused for programs sessions have used; reason and attempt logged either way) | `training.templates.configure` | built | - |
| `/api/templates/[id]/elements` | `app/api/templates/[id]/elements/route.ts` | POST every change to a draft (form or JSON): add (palette kind + index), add_library, place (drag), move, rename, remove, set_section, set_task; one transaction and one audit row each | `training.templates.configure` | built | - |
| `/api/library` | `app/api/library/route.ts` | POST create one element_library row by hand (malfunction, inject, weather, ATC line, task, block) | `training.library.manage` | built | - |
| `/templates/builder` | `app/(training)/templates/builder/page.tsx` | Kit stub; superseded by `/templates/[id]`, kept until the smoke list drops it | `training.templates.configure` | stub | `template-builder-stub` |
