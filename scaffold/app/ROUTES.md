...
| `/api/templates` | `app/api/templates/route.ts` | POST create (template + version 1 draft); archive / unarchive (ids); delete (ids + the caller's password, re-verified by Postgres; refused for programs sessions have used; attempt logged either way) | `training.templates.configure` | built | - |
